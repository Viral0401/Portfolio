  ...
  return (
    <nav className="bg-primary border-border h-16 overflow-hidden border-b">
      <div className="mx-auto flex h-full w-dvw max-w-[1200px] items-center justify-between px-4 py-1">
        {isVisible ? (
          ...
          <Link href="/">
            <div className="animate-fade-up text-primary-content relative flex items-center gap-3 transition-all duration-300 md:static">
              <Logo />
              <span className="text-primary-content">Viral Dalal</span>
            </div>
          </Link>
        )}
        ...
        <ul
          className={`${isVisible ? 'flex' : 'hidden'} animate-fade-in bg-primary absolute top-16 left-0 z-10 h-dvh w-dvw flex-col md:static md:top-0 md:flex md:h-full md:w-[80%] md:flex-row md:justify-start md:gap-4 lg:w-[70%]`}>
          {navItems.map(({ label, href }, idx) => (
            <li
              key={href}
              onClick={() => setIsVisible(false)}
              className={`border-border flex items-center border-b px-4 text-2xl md:border-y-0 md:border-e md:text-base md:first:border-s md:last:border-none md:last:px-0 lg:px-6 ${idx > 0 ? 'md:ml-2' : ''}`}>
              <Link
                href={href}
